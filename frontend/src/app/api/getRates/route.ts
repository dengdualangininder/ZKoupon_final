...
export const POST = async (request: Request) => {
  ...
  try {
    // usdc
    const USDCres = await axios.get(`https://sheets.googleapis.com/v4/spreadsheets/1VnZjhKY6QCNYrc9lcSHd3BO1Ws_sXc0jyPNvlwbJhkE/values/coinbase!H4:I4?key=${process.env.GOOGLE_API_KEY}`);
    const usdcToLocal = USDCres.data.values[0][sheetIndex];
    // usd
    const USDres = await axios.get(`https://sheets.googleapis.com/v4/spreadsheets/1AYtFng--zDWlSJmQuF16dyXwdxvrsM6SItTTYSpVq0s/values/Sheet1!B2:E2?key=${process.env.GOOGLE_API_KEY}`);
